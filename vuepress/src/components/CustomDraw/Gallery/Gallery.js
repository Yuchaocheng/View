import { grid } from '@/common/utils.js';
...
export default class Gallery {
  constructor(canvas, options) {
    ...
    // 不设置则居中
    if (!options.initX) {
      this.options.initX = (canvasWidth - this.options.width) / 2;
    }
    if (!options.initY) {
      // 上下各为图形间间隔的两倍
      const nums = this.DefaultOrder.length;
      let gapY = (canvasHeight - this.options.height * nums) / (nums + 1);
      this.options.itemGapY = gapY;
      this.options.initY = gapY;
    }
  ...
  drawIcon(type, order, params = []) {
    const { DefaultOrder, shadowTypeList, aIconList, options } = this;
    ...
    let x = options.initX;
    let y = options.initY + order * (options.height + options.itemGapY);
    let hasShadow = shadowTypeList.includes(type);
    this.baseRect([x, y], hasShadow);
    this[type].apply(this, [x, y, ...params]);
    iconListUpdate(aIconList, 'type', { x, y, w: options.width, h: options.height, type, params });
  }
  // 基础框
  ...
  line(x, y, horizontalOffset = 4) {
    ...
    this.context.lineTo(x + options.width - offset, y + options.height - offset);
    this.context.stroke();
  }
  // 矩形
  rect(x, y, insideOffset = 4) {
    ...
    this.context.strokeRect(x + offset, y + offset, options.width - 2 * offset, options.height - 2 * offset);
    this.context.stroke();
  ...
  circle(x, y, gap = 4) {
    const { options } = this;
    const maxR = Math.floor(Math.min(options.width, options.height) / 2);

    const r = maxR - gap;
    this.context.beginPath();
    this.context.save();
    let offsetX = x + options.width / 2;
    let offsetY = y + options.height / 2;
  ...
  eraser(x, y) {
    ...
    this.context.save();
    this.context.translate(x, y);
    let offsetX = options.width / 2;
    let offsetY = options.height / 2;
    this.context.beginPath();
    this.context.arc(offsetX, offsetY, 18, 0, Math.PI * 2);
    this.context.strokeStyle = 'rgba(100, 140, 200, 0.5)';
    this.context.stroke();
    /* clip()裁剪后不能访问裁剪区域的其他区域（只能渲染在裁剪区域）。clip()能限定显示区域 */
    this.context.clip();
    this.grid(0, 0, options.width, options.height, 4);
    this.context.restore();
  }
  ...
  dealMouseDown() {
    ...
    this.selectedType = curIcon.type;
    context.save();
